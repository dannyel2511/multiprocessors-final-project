...
void cipher_control(byte *file_in, byte *file_out, long long file_size, unsigned long blocks, byte *expanded_key) {
   unsigned long block;
   int padding, res;
...
int main(int argc, char *argv[]) {
   ...
   // Copy data from precomputed matrices
    ...
    // Measure time
    double time_taken = 0;
    // Copy the variable, to avoid loosing data when running several times (debugging purposes)
    file_in_cpy = (byte*)malloc(file_in_size * byte_size);
    for(int i = 0; i < 10; i++) {
      memcpy(file_in_cpy, file_in, file_in_size * byte_size); // Debug purposes
      start_timer();
      cipher_control(file_in_cpy, file_out + 1, file_in_size, blocks, expanded_key);
